...
class SpatialOrientation(ClassWithParameterSet):
    ...
    def _initCameraMatrix(self, height=None, tilt_angle=None, roll_angle=None):
        ...
        self.R_x = np.array([[1, 0, 0],
                   [0, np.cos(teta_x), -np.sin(teta_x)],
                   [0, np.sin(teta_x), np.cos(teta_x)]])

        self.R_y = np.array([[np.cos(teta_y), 0, np.sin(teta_y)],
                    [0, 1, 0],
                    [-np.sin(teta_y), 0, np.cos(teta_y)]])

        self.R_z = np.array([[np.cos(teta_z), -np.sin(teta_z), 0],
                    [np.sin(teta_z), np.cos(teta_z), 0],
                    [0, 0, 1]])
        self.R = np.dot(np.dot(self.R_x, self.R_y), self.R_z)
